...
function CarbonReductionChart() {
  ...
  const groupWeekly = (data) => {
    ...
    data.forEach((item) => {
      ...
      daySums[day] += item.totalCarbonReduction || 0;
    });
  ...
  const groupMonthly = (data) => {
    ...
    data.forEach((item) => {
      ...
      monthSums[m] += item.totalCarbonReduction || 0;
    });
  ...
  const groupYearly = (data) => {
    ...
    data.forEach((item) => {
      ...
      yearSums[y] += item.totalCarbonReduction || 0;
    });
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <h2 style={{ marginBottom: "1rem", fontWeight: "bold" }}>탄소배출감소량</h2>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          />
          <YAxis
            label={{ value: "감소량", angle: -90, position: "insideLeft" }}
          />
          <Tooltip />
          <Line
            type="monotone"
